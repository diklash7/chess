...
function getAllPossibleCoordsRook(pieceCoord) {
    ...
    for (var k = pieceCoord.i; k <= 7; k++) {
        nextCoord = { i: k, j: pieceCoord.j };
        if (isEmptyCell(nextCoord)) res.push(nextCoord);
    }
    for (var k = pieceCoord.i; k >= 0; k--) {
        nextCoord = { i: k, j: pieceCoord.j };
        if (isEmptyCell(nextCoord)) res.push(nextCoord);
    }
    for (var k = pieceCoord.j; k <= 7; k++) {
        nextCoord = { i: pieceCoord.i, j: k };
        if (isEmptyCell(nextCoord)) res.push(nextCoord);
    }
    for (var k = pieceCoord.j; k >= 0; k--) {
        nextCoord = { i: pieceCoord.i, j: k };
        if (isEmptyCell(nextCoord)) res.push(nextCoord);
    }
    return res;
